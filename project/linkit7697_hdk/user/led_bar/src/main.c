...
static void pin_set_output(ledbar_pin_t port, bool stat)
{
    hal_gpio_set_output((hal_gpio_pin_t)port.pin, stat == true ? HAL_GPIO_DATA_HIGH : HAL_GPIO_DATA_LOW);
}
...
static void LEDBar_example(void)
{
    ledbar_pin_t clk;
    ledbar_pin_t dat;
    ledbar_t *ledbar;
    ...
    hal_gpio_init(HAL_GPIO_33);   // p5 clk
    hal_gpio_init(HAL_GPIO_34);   // p4 dat
    ...
    clk.pin = (uint32_t)HAL_GPIO_33;
    dat.pin = (uint32_t)HAL_GPIO_34;

    ledbar = LEDBAR_New(pin_set_output, delay_ms, clk, dat, false);

    while(1) {

        ledbar->opt->setLevel(ledbar, 2);
        state = ledbar->opt->getBits(ledbar);

        for(uint16_t i = 0; i <= 1023; i++) {
            state <<= 1;
            printf("%x\n", state);
            // ledbar->opt->setLevel(ledbar, i);
            ledbar->opt->setBits(ledbar, i);
            hal_gpt_delay_ms(50);
        }
    }
